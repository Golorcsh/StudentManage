...
EditStuMessBox::EditStuMessBox(QWidget *parent, M_Sql *sql) : QDialog(parent), sql_(sql), isAdd_(true) {
  /*!
  ...
  //信号与槽
  //提交按钮,将输入信息保存到文件中
  connect(submit_, &QPushButton::clicked, this, &EditStuMessBox::SaveStuMessToDB);
  //取消按钮,关闭窗口
...
void EditStuMessBox::SaveStuMessToDB() {
  if (stu_name_->text().isEmpty() || stu_age_->text().isEmpty() || stu_gender_->text().isEmpty()) {
...
void EditStuMessBox::UpDateStuMessToDB() {
  if (stu_name_->text().isEmpty() || stu_age_->text().isEmpty() || stu_gender_->text().isEmpty()) {
    ...
    //将学生信息插入数据库中
    QStringList string_list;
    string_list << id_ << stu_name_->text() << stu_gender_->text() << stu_age_->text();
...
QString EditStuMessBox::GetStuName() const {
  return stu_name_->text();
}
void EditStuMessBox::SetStuName(const QString &name) {
  stu_name_->setText(name);
}
QString EditStuMessBox::GetStuAge() const {
  return stu_age_->text();
}
void EditStuMessBox::SetStuAge(const QString &age) {
  stu_age_->setText(age);
}
QString EditStuMessBox::GetStuGender() const {
  return stu_gender_->text();
}
void EditStuMessBox::SetStuGender(const QString &gender) {
  stu_gender_->setText(gender);
}
void EditStuMessBox::SetId(const QString &id) {
  id_ = id;
}
/*!
...
void EditStuMessBox::SetModifyMode() {
  ...
  connect(submit_, &QPushButton::clicked, this, &EditStuMessBox::UpDateStuMessToDB);
}
